...
def _handle_db_error(exc: Exception) -> None:
    st.error("Database error. The app can't load your vault right now.")
...
def main() -> None:
    ...
    if "user_id" not in st.session_state:
        st.session_state["user_id"] = _db_call(
            REPO.ensure_user_from_auth0,
            auth0_sub=auth0_sub,
            email=email,
    ...
    if filtered:
        ...
        with st.expander("Remove records"):
            ...
            if st.button("Delete selected records", type="primary", disabled=not remove_options):
                ...
                removed = _db_call(REPO.delete_records, user_id=user_id, record_ids=ids)
